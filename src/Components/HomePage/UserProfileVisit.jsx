...
const UserProfileVisit = () => {
  ...
  const fetchPublicBoards = async () => {
    try {
      ...
      console.log("paranms   name :::", params.name);
      ...
      if (data?.data?.param) {
        console.log("redirected", data.data.param);
        navigate(`/${data?.data?.param}`);
      }
  ...
  console.log("all board data ", allBoards);

  const fetchCollectionsBoardData = async (url) => {
    const apiUrl = `${baseURL}` + `${url}`;
    const headers = new Headers({
      "Content-Type": "application/json",
    });
      ...
      <div className="flex flex-col lg:flex-row">
        <div className="lg:w-[30%] bg-[#2D2D2D]">
          <div className="flex m-2 justify-between bg-[#2D2D2D] items-center ">
            <p className=" text-[#BFBFBF] pl-2 mt-2">
              All Boards /{" "}
              {collectionBoardData
                ? collectionBoardData?.boards?.length
                : allBoards?.length}
            </p>
                  ...
                  {!allBoards?.length ? (
                    ...
                    allBoards.map((board, boardIndex) => (
                      <div key={boardIndex} className="mb-2">
                        ...
                        >
                          <div className="w-2 h-2 rounded-lg absolute top-0 right-0"></div>
            ...
            {manageCreator && (
              <Suspense fallback={<div>Loading Manage Creator...</div>}>
                <div className="flex justify-center w-full h-full bg-[#2A2A2A]">
                  ...
                  {/* <div>Manage creator</div> */}
                  {console.log("manageCreator", manageCreator)}
                </div>
              </Suspense>
            )}
          </div>
